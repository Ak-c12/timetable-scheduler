...
@app.route("/admin/departments", methods=["GET", "POST", "OPTIONS"])
@token_required
@admin_required
...
@app.route("/admin/rooms", methods=["GET", "POST", "OPTIONS"])
@token_required
@admin_required
...
@app.route("/admin/sections", methods=["GET", "POST", "OPTIONS"])
@token_required
@admin_required
...
@app.route("/admin/students/<int:student_id>", methods=["GET", "PUT", "DELETE", "OPTIONS"])
@token_required
@admin_required
...
@app.route("/admin/students/delete_bulk", methods=["POST", "OPTIONS"])
@token_required
@admin_required
...
@app.route("/upload/faculty", methods=["POST", "OPTIONS"])
@token_required
@admin_required
...
@app.route("/upload/students", methods=["POST", "OPTIONS"])
@token_required
@admin_required
...
@app.route("/rooms/status", methods=["GET", "OPTIONS"])
@token_required
...
@app.route("/teacher/timetable", methods=["GET", "OPTIONS"])
@token_required
...
@app.route("/student/timetable", methods=["GET", "OPTIONS"])
@token_required
...
# ---------- CSV UPLOAD ROUTES ----------
@app.route("/upload/departments", methods=["GET", "POST", "OPTIONS"])
@token_required
...
@app.route("/upload/sections", methods=["GET", "POST", "OPTIONS"])
@token_required
...
@app.route("/add_department", methods=["GET", "POST", "OPTIONS"])
def add_department():
    try:
        if request.method != "POST":
...
@app.route("/get_departments", methods=["GET", "OPTIONS"])
def get_departments():
    try:
        depts = Department.query.all()
        return jsonify([{"id": d.id, "dept_name": d.dept_name} for d in depts])
...
@app.route("/add_section", methods=["GET", "POST", "OPTIONS"])
def add_section():
    try:
        if request.method != "POST":
...
@app.route("/get_sections", methods=["GET", "OPTIONS"])
def get_sections():
    try:
        dept_name = request.args.get("dept_name")
        year = request.args.get("year")
...
@app.route("/add_faculty", methods=["GET", "POST", "OPTIONS"])
def add_faculty():
    try:
        if request.method != "POST":
...
@app.route("/get_faculty", methods=["GET", "OPTIONS"])
def get_faculty():
    try:
        faculty = Faculty.query.all()
        return jsonify([
...
@app.route("/add_course", methods=["GET", "POST", "OPTIONS"])
def add_course():
    try:
        if request.method != "POST":
...
@app.route("/get_courses", methods=["GET", "OPTIONS"])
def get_courses():
    try:
        dept_name = request.args.get("dept_name")
        year = request.args.get("year")
...
@app.route("/add_room", methods=["GET", "POST", "OPTIONS"])
def add_room():
    try:
        if request.method != "POST":
...
@app.route("/get_rooms", methods=["GET", "OPTIONS"])
def get_rooms():
    try:
        rooms = Classroom.query.all()
        return jsonify([
...
@app.route("/set_course_allocation", methods=["GET", "POST", "OPTIONS"])
...
@app.route("/get_course_allocations", methods=["GET", "OPTIONS"])
...
@app.route("/generate_timetable", methods=["GET", "POST", "OPTIONS"])
...
@app.route("/generate_csvs", methods=["GET", "POST", "OPTIONS"])
def generate_csvs():
    try:
        if request.method != "POST":
    ...
    try:
        if current_user.role == "student":
            ...
            for day, classes in schedule.items():
                response += f"**{day}:**\n"
                for cls in classes:
                    response += f"🕐 {cls['time']} - **{cls['course']}** ({cls['course_code']})\n"
